...
function updatePagination(swiper) {
    ...
    const formettedNum = curentNum < 10 ? `0${curentNum}` : curentNum;
    document.querySelector('.swiper-pagination-current').textContent = formettedNum;
}
...
visionCards.forEach((card, i) => {
    const tl = gsap.timeline({
        scrollTrigger: {
            trigger: '.vision',
            start: `top+=${i * 300} center`, //index*300, 300px에서 시작 | top+= 붙여쓰기
            end: `top+=${(i + 1) * 300} center`, // 600px에서 끝
            scrub: 1.2,
        }
    });
...
gsap.timeline({
    scrollTrigger: {
        trigger: '.vid_box',
        start: 'top 10%',
        end: "top 10%+=1500", //1500px 스크롤동안 애니메이션 진행
        scrub: 1,
        pin: '.vid_box', //고정 
        pinSpacing: true,
...
}).fromTo(cards, { y: 300, z: 200, opacity: 0.8 },{ y: 0, z: 0, duration: 1, ease: "power2.out", stagger: 1 ,opacity: 1 })//stagger : 카드 하나씩 1초 간격으로 차례차례 등장
